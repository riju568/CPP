...
// Generic Sorting Algorithms (Chapter 23)


// 23.1 Generic Bubble Sort
...
void bubble_sort(std::vector<T>& vec, Compare comp = Compare()) {
    ...
    for (size_t k = 0; k < vec.size() && need_next_pass; ++k) {
        ...
        for (size_t i = 0; i < vec.size() - 1 - k; ++i) {
            if (comp(vec[i + 1], vec[i])) { // Custom comparator ordering check
...
// 23.4 Median-of-Three Pivot Selection helper for Quick Sort
...
// 23.3 & 23.4 Improved Generic Quick Sort
template <typename T, typename Compare>
size_t partition(std::vector<T>& vec, size_t low, size_t high, Compare comp) {
    ...
    std::swap(vec[median_idx], vec[high]); // Move pivot to end
...
template <typename T, typename Compare>
void quick_sort_impl(std::vector<T>& vec, int low, int high, Compare comp) {
    if (low < high) {
        size_t p = partition(vec, static_cast<size_t>(low), static_cast<size_t>(high), comp);
        quick_sort_impl(vec, low, static_cast<int>(p) - 1, comp);
        quick_sort_impl(vec, static_cast<int>(p) + 1, high, comp);
    }
}
...
void quick_sort(std::vector<T>& vec, Compare comp = Compare()) {
    if (vec.size() <= 1) return;
    quick_sort_impl(vec, 0, static_cast<int>(vec.size()) - 1, comp);
}

// 23.5 Generic Heap Sort
template <typename T, typename Compare>
...
void heap_sort(std::vector<T>& vec, Compare comp = Compare()) {
    ...
    for (int i = static_cast<int>(vec.size() / 2) - 1; i >= 0; --i) {
        heapify(vec, vec.size(), static_cast<size_t>(i), comp);
    }
...
int main() {
    ...
    try {
        ...
        while (running) {
            ...
            std::cout << "   ~ Chapter 23: Sorting Algorithms Menu ~   \n";
            ...
            if (std::cin >> choice) {
                ...
                std::cout << "Error: Invalid selection. Enter an integer.\n";
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            }
        }
