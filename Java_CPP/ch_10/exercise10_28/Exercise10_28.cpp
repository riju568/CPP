 ...
 * Custom MyStringBuilder2 class translated directly to safe modern C++.
 */
...
public:
    ...
    MyStringBuilder2 &insert(int offset, const MyStringBuilder2 &s)
    {
        ...
        if (off > 0)
        {
            std::memcpy(nuValues.get(), values.get(), off);
        }
        if (s.len > 0)
        {
            std::memcpy(nuValues.get() + off, s.values.get(), s.len);
        }
        if (len - off > 0)
        {
            std::memcpy(nuValues.get() + off + s.len, values.get() + off, len - off);
        }

        values = std::move(nuValues);
        len = newLength;
        return *this;
    }
    ...
    [[nodiscard]] MyStringBuilder2 toUpperCase() const
    {
        std::string upperStr;
        upperStr.reserve(len);
        for (std::size_t i = 0; i < len; ++i)
        {
            upperStr.push_back(static_cast<char>(std::toupper(static_cast<unsigned char>(values[i]))));
        }
        return MyStringBuilder2(upperStr);
    }
