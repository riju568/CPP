...
int main()
{
    try
    {
        ...
        std::cin.tie(nullptr);
        MyString2 s1("HelloWorld");
        MyString2 s2("MILO");
        std::cout << "s1 string: " << s1.get_str() << '\n';
        std::cout << "s1.substring(5): " << s1.substring(5).get_str() << '\n';
        std::cout << "s2.to_upper_case().to_chars(): " << vector_to_array_string(s2.to_upper_case().to_chars()) << '\n';
        std::cout << "s1.compare(\"HelloWorld\"): " << s1.compare("HelloWorld") << '\n';
        std::cout << "MyString2::value_of(true): " << MyString2::value_of(true).get_str() << '\n';
        return EXIT_SUCCESS;
    }
